...
//-----------------edit by chy
    ...
    int sectorsGet[NumDirect + L1_INDEX_NUM*L1_INDEX_NUM];
    int getNum = 0;
    L1Index = -1;
    L2Index = -1;
/*
...
*/
    ...
    if(numSectors > NumDirect)
    {
        if(numSectors > NumDirect + L1_INDEX_NUM)
        {
            ...
            {
                result = false;
            }//exceed the max length of sectors
            else//L2 needed
            {
                adSector = divRoundUp((numSectors - NumDirect -L1_INDEX_NUM),L1_INDEX_NUM);
                if(freeMap->NumClear() <= NumDirect + adSector + 1)
                {
                    result = false;
                }
                else if(freeMap->NumClear() >= NumDirect+adSector+1)
                {
                    if(freeMap->NumClear() <= NumDirect+L1_INDEX_NUM+1)
                        result = false;
                    else if(freeMap->NumClear() >NumDirect+L1_INDEX_NUM+1)///L2
                    {
                        for( int  i = 0; i < numSectors + adSector +2;i++)
                        {
        ...
        else if(numSectors <=NumDirect + L1_INDEX_NUM) //noly L1 needed
        {
            if(freeMap->NumClear() < numSectors+1)
                result = false;
            else if(freeMap->NumClear() >=numSectors+1)//L1 here
            {
...
//----realy allocate the sectors get in the sectorsGet[]
    ...
    if(curSector < numSectors)
    {
        ...
        //write to the disk sector
        synchDisk->WriteSector(L1Index,(char *)hdrL1Buf);

        if(curSector < numSectors)//L2 here
        {
...
void
FileHeader::AllocateToDataSectors(BitMap* freebitmap, int numSectors, int dataSectors[])
{
    for(int i = 0; i< numSectors; i++)
    {
        dataSectors[i] = freebitmap->Find();
    }
}
*/
