...
ExceptionHandler(ExceptionType which)
{
    ...
    else if( (which == PageFaultException))
    {
        ...
        int address = machine->ReadRegister(BadVAddrReg);
        machine->TranslatePTE(address);
    } else if (which == PTEPageFaultException ) { ///here nedded to add the processing part switch 
        DEBUG('a', "PageFault on PTE.calling ASSERT()\n");
        machine->UpdatePageTable();
        //ASSERT(FALSE);
    }
    else {
