...
{ 
  ...
  readyList_1 = new List();
  readyList_2 = new List();
  lastSwitchTime = 0;
  policy = SCHED_PRIO_P;
} 
...
void static SchedInterruptHandler( int dummy )
{
  ...
  case SCHED_PRIO_P:   //  Premptive     Priority scheduling (MP1)
    ...
    break;
     ...
     case SCHED_MLFQ:     // 5.3.6 Multilevel feedback queue scheduling
       {
	 ASSERT(thread->getLevel()>=0&&thread->getLevel()<=2);
	 if(thread->getLevel() == 0){thread->setTimeSlice ( 3); readyList->Append((void*)thread);}
	 else if(thread->getLevel() == 1)
	   {
	     if(thread->getPreempted())readyList_1->Prepend((void*)thread);
	     else{
	       if(thread->getTimeLeft()<=4)thread->setTimeSlice( thread->getTimeLeft());
	       else thread->setTimeSlice(4);readyList_1->Append((void*)thread);
	     }
	   }
	 else if(thread->getLevel() == 2)
	   {
	     if(thread->getPreempted())
	       readyList_2->Prepend((void*)thread);
	     else readyList_2->Append((void*)thread);
	   }
	 break;
       }
     default:
  ...
  if(readyList->IsEmpty())
    {
      ...
      if(readyList_1->IsEmpty())
	{
	  if(readyList_2->IsEmpty())return NULL;
	  else return (Thread*)readyList_2->Remove();
	}
      else return (Thread*)readyList_1->Remove();
	     // return NULL;
    }
...
interrupt->Schedule(SchedInterruptHandler, (int) this,currentThread->getTimeSlice()*10,TimerInt);  
    ////-------------------------------------------------------
...
//----------------------------------------------
//Scheduler::CheckIfHigherEnter
//   check for the MLFQ
//-----------------------------
bool
Scheduler::CheckIfHigherEnter()
{
  if(currentThread->getLevel() == 1)
    {
      if(readyList->IsEmpty())
	return false;
      else
	{
	  currentThread->setPreempted(true);
	  return true;
	}
    }
  else if(currentThread->getLevel() == 2)
    {
      if(readyList->IsEmpty()&& readyList_1->IsEmpty())
	return false;
      else
	{
	  currentThread->setPreempted(true);
